...
public record TeamInfoResponse(
		...
		@Schema(description = "팀 이름", example = "teamName") String teamName,
		@Schema(description = "팀 현재 인원수", example = "4") int currentMembers,
		@Schema(description = "팀 최대 인원수", example = "10") int maxMembers,
		@Schema(description = "팀 설명", example = "teamDescription") String Description) {
	...
	public static List<TeamInfoResponse> fromList(List<Team> teams) {
		return teams.stream().map(TeamInfoResponse::from).toList();
	}
}
