...
public record CommitHistoryResponse(
		@Schema(description = "커밋 제목") String title,
		@Schema(description = "커밋히스토리 ID") Long id,
		@Schema(description = "커밋 설명") String description,
		@Schema(description = "깃허브 링크") String githubLink,
		@Schema(description = "깃허브 아이디") String githubId,
		@Schema(description = "커밋 날짜") LocalDateTime createdAt) {
	...
	public static List<CommitHistoryResponse> fromList(List<CommitHistory> commitHistories) {
		return commitHistories.stream().map(CommitHistoryResponse::from).toList();
	}
}
