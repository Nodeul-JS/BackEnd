...
public interface CommitHistoryRepository extends JpaRepository<CommitHistory, Long> {
	@Query(
			"SELECT c FROM CommitHistory c WHERE c.user.githubId = :githubId AND DATE(c.createdAt) = CURRENT_DATE")
	...
	List<CommitHistory> findByUserAndCreatedAtBetweenOrderByCreatedAtAsc(
