...
public class TeamService {
	...
	public void createGroup(TeamCreateRequest dto) {
		...
		Team team =
				Team.createWithLeader(dto.getTeamName(), dto.getMaxMember(), dto.getDescription(), user);
		teamRepository.save(
				team); // Team Constructor: Protected -> have to use save method(team.set...)
		...
		Team team =
				teamRepository
						...
						.orElseThrow(
								() ->
										new IllegalArgumentException(
												"Invalid team ID: There is No ID. Here is Service"));
		User user =
				userRepository
						.findById(userId)
						.orElseThrow(
	...
	public MemberInviteResponse inviteMember(MemberInviteRequest dto) {
		User user =
				userRepository
						.findByGithubId(dto.getGithubId())
						.orElseThrow(
								() ->
										new IllegalArgumentException(
												"Invalid user ID!! : There is No ID. Here is Service"));
